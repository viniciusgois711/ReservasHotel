...
public class HospedeDB {
    ...
    public void editarHospede(int id, Hospede hospede){
        try(Connection conexao = getConnection();
            PreparedStatement statement = conexao.prepareStatement("update hospedes set nome = ?, cpf = ?, telefone = ? where id = ?")
        ){
            statement.setString(1, hospede.getNome());
            statement.setString(2, hospede.getCpf());
            statement.setString(3, hospede.getTelefone());
    ...
    public void deletarHospede(int id){
        ...
        try(Connection conexao = getConnection();
            PreparedStatement statement = conexao.prepareStatement("delete from hospedes where id = ?")
        ){
